...
# Manually Enter Execution Times
...
# Step 4: Execution Time Bar Chart
...
plt.ylim(0, max(execution_times) * 1.2)  # Adjusts y-axis for better visualization
plt.savefig("execution_time_comparison.png", dpi=300)  # Save as PNG
plt.close()
...
plt.savefig("speedup_comparison.png", dpi=300)  # Save as PNG
plt.close()

# Line Graph (Speedup Scaling)
plt.figure(figsize=(8,5))
...
plt.legend()
plt.grid(True)
plt.savefig("speedup_scaling.png", dpi=300)  # Save as PNG
plt.close()
